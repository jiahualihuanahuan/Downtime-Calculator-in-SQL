...
SELECT 
  b.t_stamp Down_begin, 
  a.t_stamp Down_end, 
  timestampdiff(minute, b.t_stamp, a.t_stamp) Down_Time_minute, 
  REPLACE(REPLACE(b.009_Line_2_Infeed_Full,0,'FCB'),1,'ROBOT') AS Caused_by
...
# Table Name: PE06 (photo eye number 6)
# Columns Names: index, 006_Line_1_Infeed_Full, timestamp
# The PE06 sensor insert a new row of data every time when sensor was triggered i.e. nothing = 0, case present = 1
# it record a unique index number in index column ascendingly, 0 or 1 in 006_Line_1_Infeed_Full column, and a timestamp of current time
...
SELECT 
  ...
  a.t_stamp Down_end, 
  timestampdiff(minute, b.t_stamp, a.t_stamp) Down_Time_minute, 
  REPLACE(REPLACE(b.006_Line_1_Infeed_Full,0,'FCB'),1,'ROBOT') AS Caused_by
